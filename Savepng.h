...
/*
	...
	PNG 文件由多个 chunk 组成。每个 chunk 的类型以 4 个字符表示。最基本的 PNG 文件内容是：
	...
	chunk 内容
	chunk 的 CRC（包括类型和内容）
	PNG 里的数据是以大端（big endian）编码的，但在 IDAT 中，每个 block 的长度则以小端存储。
	另外，实现的难点之一，是要同时实现 CRC-32 及 Adler-32 校验和（checksum）的生成。
*/
